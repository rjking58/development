...
char  *CLO_includedir = NULL;  /* command line option includedir */
...
#ifndef _MSC_VER
#if !(defined(mips) && defined(SYSTYPE_SYSV) || defined(svr4))
...
#else
	/* since NT has files as CR/LF, but strips the CR's on 
	 * input, we have to adjust between what is returned by
	 * fstat and the actual number of bytes read by the _read
	 * function... egad  rjk 
  	 */
   bytes_read = _read(fd, content->f_base, st.st_size);
   byte_diff = 0;
	for (byte_ofst = 0; byte_ofst < bytes_read; byte_ofst++)
	{
		if (content->f_base[byte_ofst] == LINE_FEED)
		{
			byte_diff++;
		}
	}
	if ( (bytes_read + byte_diff) != st.st_size)
	{
		log_fatal("cannot read all of %s\n", file);
	}
	else
...
/*VARARGS*/
void log_fatal(char* x0, ...)
{
	va_list pvar;
	va_start(pvar, x0);
	vfprintf(stderr, x0, pvar);
	va_end(pvar);
	exit (1);
}
