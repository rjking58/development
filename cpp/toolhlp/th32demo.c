...
DWORD xor_value;        // Magic XOR value to convert IDs to addresses
...
void Handle_WM_INITDIALOG(HWND hWndDlg)
{
    ...
    // calculate the magic xor_value we need to convert heap IDs to
    // linear addresses.
    hSnapshot = CreateToolhelp32Snapshot( TH32CS_SNAPHEAPLIST, 0 );
    hl32.dwSize = sizeof(hl32);
    Heap32ListFirst( hSnapshot, &hl32 );
    xor_value = (DWORD)GetProcessHeap() ^ hl32.th32HeapID;
...
void UpdateHeapsListbox( HWND hWndDlg, DWORD processID )
{
    ...
    {
        unsigned lbIndex;
        lbIndex = lbprintf( hWndHeapsLb,"ID:%08X\tAddr:%08X\t%s %s",
                            hl32.th32HeapID, hl32.th32HeapID ^ xor_value,
