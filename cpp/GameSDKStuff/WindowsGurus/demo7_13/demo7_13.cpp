...
{
   LPDIRECTDRAWSURFACE7 frames[6]; // 3 frames of animation for complete walk cycle
...
void reverse_frames(LPDIRECTDRAWSURFACE7 lpdds_src, LPDIRECTDRAWSURFACE7 lpdds_tgt,int cx, int cy);

LPDIRECTDRAWSURFACE7 DDraw_Create_Surface(int width, int height, int mem_flags, int color_key);

int SurfaceToSurfaceBLT(LPDIRECTDRAWSURFACE7 source, 
                        int x, 
                        int y, 
                        int width, 
                        int height, 
...
DDSURFACEDESC2        ddsd2;                  // a direct draw surface description struct
...
int SurfaceToSurfaceBLT(LPDIRECTDRAWSURFACE7 source, // source surface to draw
...
LPDIRECTDRAWSURFACE7 DDraw_Create_Surface(int width, int height, int mem_flags, int color_key = 0)
{
   ...
   ddsd.ddsCaps.dwCaps = DDSCAPS_VIDEOMEMORY | mem_flags;
...
int Game_Main(void *parms = NULL, int num_parms = 0)
{
   ...
   static bool is_still = true;
   // lookup for proper walking sequence
   static int animation_seq[8] = {0,1,0,2,3,4,3,5};
   ...
   SurfaceToSurfaceBLT(lpddsbackground,0,0, SCREEN_WIDTH,SCREEN_HEIGHT, lpddsback,0);    
   ...
   for (index=0; index < 3; index++)
    {
      ...
      if (aliens[index].x < -80)
      {
         aliens[index].x = SCREEN_WIDTH;
      }
      
      if (aliens[index].velocity != 0)
      {
         if (aliens[index].velocity > 0)
         {
            // animate bot
         ...
         else
         {
            ...
            if (++aliens[index].counter >= (8 + aliens[index].velocity))
            {
               ...
               // advance to next frame
               if (++aliens[index].current_frame > 7)
                  aliens[index].current_frame = 4;
   ...
   {
     // draw objects
     SurfaceToSurfaceBLT(aliens[index].frames[animation_seq[aliens[index].current_frame]], 
...
int Game_Init(void *parms = NULL, int num_parms = 0)
{
   ...
   // now reverse the frames ...
   for (int index = 0; index < 3; index++)
   {
      // create surface to hold image
      aliens[0].frames[index+3] = DDraw_Create_Surface(72,80,0);
      
      // now load bits...
      reverse_frames(aliens[0].frames[index],    
                     aliens[0].frames[index+3],
                     72,
                     80);   
...
int Game_Shutdown(void *parms = NULL, int num_parms = 0)
{
   ...
   // kill all the surfaces (only index 0, the other indexes are copies of the orig. ptrs).
   for (int index = 0; index < 6; index++)
   {
      aliens[0].frames[index]->Release();
   }
...
void reverse_frames(LPDIRECTDRAWSURFACE7 lpdds_src, LPDIRECTDRAWSURFACE7 lpdds_tgt,int cx, int cy)
{
   // this is byte mode.. so everything matches..
   // set size of the structure
   ddsd.dwSize = sizeof(ddsd);
   ...
   UCHAR* dest_ptr = (UCHAR *)ddsd2.lpSurface;
   UCHAR* src_ptr = (UCHAR *)ddsd.lpSurface;
   for ( int curr_y = 0 ; 
         curr_y < cy    ;
         curr_y++          )
   {
      UCHAR* dest_row = dest_ptr;
      UCHAR* src_row  = src_ptr + (cx - 1);
      ...
      dest_ptr += ddsd2.lPitch;
      src_ptr += ddsd.lPitch;
   }
   lpdds_src->Unlock(NULL);
   lpdds_tgt->Unlock(NULL);
