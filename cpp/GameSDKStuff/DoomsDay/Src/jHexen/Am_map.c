...
/*
   ...
   dy = ml->a.y - ml->b.y;
   dx = ml->b.x - ml->a.x;
   if (!dy) is->islp = (dx<0?-DDMAXINT:DDMAXINT);
   else is->islp = FixedDiv(dx, dy);
   if (!dx) is->slp = (dy<0?-DDMAXINT:DDMAXINT);
...
void AM_Ticker(void)
{
	if(!automapactive || !players[consoleplayer].plr->mo)
		return;
...
void AM_drawWalls(void)
{
	...
	for(i = 0; i < numlines; i++)
	{
		...
		gl.Vertex2f(FIX2FLT(CXMTOFX(lines[i].v1->x)),
					FIX2FLT(CYMTOFX(lines[i].v1->y)) / 1.2);
		gl.Vertex2f(FIX2FLT(CXMTOFX(lines[i].v2->x)),
...
void AM_drawPlayers(void)
{
	...
	if(!netgame)
	{
		...
		AM_drawLineCharacter(player_arrow, NUMPLYRLINES, 0,
							 plr->plr->mo->angle, WHITE, plr->plr->mo->x,
							 plr->plr->mo->y);
		gl.End();
		return;
	}
	...
	for(i = 0; i < MAXPLAYERS; i++)
	{
		...
		AM_drawLineCharacter(player_arrow, NUMPLYRLINES, 0, p->plr->mo->angle,
							 their_colors[cfg.PlayerColor[i]], p->plr->mo->x,
							 p->plr->mo->y);
	}
	gl.End();
}
...
void AM_DrawDeathmatchStats(void)
{
	...
	for(i = 0; i < MAXPLAYERS; i++)
	{
		...
		else
		{
			...
			MN_DrTextA(textBuffer, j + 8, yPosition);
			yPosition += 10;
		}
	}
}
